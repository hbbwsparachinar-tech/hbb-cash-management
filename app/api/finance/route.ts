...
export async function POST(request:Request){const db=env.DB;await initialize(db);const b=await request.json() as Record<string,unknown>;if(b.kind==="location"){if(!String(b.name||"").trim())return Response.json({error:"Location name is required"},{status:400});const location=await db.prepare("INSERT INTO cash_locations(name,opening_balance) VALUES(?,?) RETURNING id,name,opening_balance AS openingBalance").bind(String(b.name).trim(),Number(b.openingBalance)||0).first();return Response.json({location},{status:201})}if(!b.date||!b.type||!b.category||!b.amount||(b.type==="Cash In"&&!b.from)||(b.type==="Cash Out"&&(!b.voucher||!b.to)))return Response.json({error:"All required fields must be completed"},{status:400});try{const transaction=await db.prepare(`INSERT INTO cash_transactions(voucher,date,type,category,amount,source,destination,description) VALUES(?,?,?,?,?,?,?,?) RETURNING ${txSelect}`).bind(b.type==="Cash Out"?String(b.voucher).trim():null,b.date,b.type,b.category,Number(b.amount),b.type==="Cash In"?String(b.from).trim():"",b.type==="Cash Out"?String(b.to).trim():"",String(b.description||"").trim()).first();return Response.json({transaction},{status:201})}catch{return Response.json({error:"Cash Out voucher / bill number must be unique"},{status:409})}}
